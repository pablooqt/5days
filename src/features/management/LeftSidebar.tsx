...
export const LeftSidebar: React.FC<LeftSidebarProps> = ({ activeTab, onTabChange, mobileOpen = true }) => {
    ...
    >
      ...
      <nav className="flex-1 py-3 space-y-0.5 px-2 overflow-y-auto">
        {NAV_ITEMS.map(({ id, icon: Icon, label }) => (
          ...
          >
            <Icon className={`w-4 h-4 shrink-0 ${activeTab === id ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
      ...
      {!collapsed && (
        <div className="border-t border-slate-100 overflow-y-auto" style={{ maxHeight: '55vh' }}>
          {activeTab === 'overview' && (
            <div className="p-3 space-y-3">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide">Building Status</p>
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-2.5 text-center">
                  <div className="text-lg font-bold text-emerald-700">{onlineCount}</div>
                ...
                <div className={`${warningCount > 0 ? 'bg-amber-50 border-amber-100' : 'bg-slate-50 border-slate-100'} border rounded-xl p-2.5 text-center`}>
                  <div className={`text-lg font-bold ${warningCount > 0 ? 'text-amber-700' : 'text-slate-400'}`}>{warningCount}</div>
                  <div className={`text-[10px] font-medium ${warningCount > 0 ? 'text-amber-600' : 'text-slate-400'}`}>Warnings</div>
                </div>
              </div>
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5">
                <div className="text-[10px] text-slate-500 mb-1 font-medium">Floors Active</div>
                <div className="flex gap-1">
                  {demoBuildingConfig.floors.map((f) => (
                    <div key={f.id} className="flex-1 h-1.5 bg-indigo-400 rounded-full" />
                  ))}
                </div>
                <div className="text-[10px] text-slate-400 mt-1">{demoBuildingConfig.floors.length} floors monitored</div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'floors' && (
            <div className="p-3 space-y-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide mb-2">Select Floor</p>
              <button
              ...
              {demoBuildingConfig.floors.map((f) => (
                ...
                >
                  <div>{f.name}</div>
                  <div className={`text-[10px] font-normal ${selectedFloorId === f.id ? 'text-indigo-200' : 'text-slate-400'}`}>
          ...
          {activeTab === 'rooms' && (
            <div className="p-3 space-y-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide mb-2">All Rooms</p>
              {demoBuildingConfig.floors.flatMap((f) =>
                f.rooms.map((r) => (
                  <button
                    key={r.id}
                     onClick={() => { selectFloor(r.floorId); setFloorMode('isolate'); selectRoom(r.id); issueCameraCommand('focusRoom', r.id); }}
                  ...
                  >
                    ...
                    <div className={`text-[10px] ${selectedRoomId === r.id ? 'text-indigo-200' : 'text-slate-400'}`}>
          ...
          {activeTab === 'devices' && (
            <div className="p-3 space-y-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide mb-2">All Devices</p>
              {allDevices.map((def) => {
                ...
                return (
                  <button
                   ...
                   onClick={() => {
                     ...
                     selectDevice(def.id);
                     issueCameraCommand('focusFloor', def.floorId);
                   }}
                    className="w-full text-left px-3 py-2 rounded-xl text-xs transition-all cursor-pointer hover:bg-slate-50 text-slate-700 flex items-center gap-2"
                  >
                    <span className={`w-2 h-2 rounded-full shrink-0 ${dotColor}`} />
                    <span className="font-mono font-semibold truncate flex-1">{def.id}</span>
                    {def.status === 'warning' && (
                      <span className="text-[9px] bg-amber-100 text-amber-700 px-1 py-0.5 rounded font-bold">!</span>
                    )}
                  </button>
                );
              })}
          ...
          {activeTab === 'activity' && (
            <div className="p-3 space-y-2">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide mb-2">Recent Activity</p>
              {[
                { time: 'Just now',   msg: 'AC_LOBBY temperature set to 23°C', type: 'control' },
                { time: '2m ago',     msg: 'SENSOR_102 threshold warning triggered', type: 'alert' },
                { time: '5m ago',     msg: 'DOOR_201 unlocked by operator', type: 'control' },
                { time: '12m ago',    msg: 'LIGHT_LOBBY brightness set to 80%', type: 'control' },
                { time: '18m ago',    msg: 'CCTV_301 went offline', type: 'alert' },
                { time: '25m ago',    msg: 'Building digital twin synced', type: 'system' },
              ].map((item, i) => (
                <div key={i} className="flex gap-2">
                  <span className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${item.type === 'alert' ? 'bg-amber-400' : item.type === 'control' ? 'bg-indigo-400' : 'bg-slate-300'}`} />
                  <div>
          ...
          {activeTab === 'settings' && (
            <div className="p-3 space-y-3">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wide mb-2">Settings</p>
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-xs text-slate-600">Auto-focus on select</span>
                <div className="w-8 h-4 bg-indigo-600 rounded-full relative">
                  <div className="absolute right-0.5 top-0.5 w-3 h-3 bg-white rounded-full" />
                </div>
              </label>
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-xs text-slate-600">Show device markers</span>
                <div className="w-8 h-4 bg-indigo-600 rounded-full relative">
                  <div className="absolute right-0.5 top-0.5 w-3 h-3 bg-white rounded-full" />
                </div>
              </label>
              ...
              <div className="pt-2 border-t border-slate-100">
                ...
                <div className="mt-1 space-y-0.5 text-[10px] text-slate-500">
                  ...
                  <div className="flex justify-between"><span>F</span><span>Focus room</span></div>
                  <div className="flex justify-between"><span>T</span><span>Transparent walls</span></div>
                  <div className="flex justify-between"><span>0–3</span><span>Floor shortcuts</span></div>
                </div>
              </div>
            </div>
          )}
